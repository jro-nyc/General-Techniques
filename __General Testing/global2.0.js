...
"mvt-no");else this.SC(c,b?"mvt-yes":"mvt-no",e,d);}}if(b){	var u={"src":this.S()};this.XH(u);}}},XH:function(u){
if(typeof(u)!="object")return;var s=this.D.createElement(this.ST);for(n in this.SA){if(!this.isIE6)s.setAttribute(
n,this.SA[n]);else s[n]=this.SA[n];}for(n in u){if(!this.isIE6)s.setAttribute(n,u[n]);else s[n]=u[n];}var h=
this.D.getElementsByTagName("head");if(h[0])h[0].insertBefore(s,h[0].childNodes[h[0].childNodes.length-1]);else 
this.D.body.insertBefore(s,this.D.body.childNodes[D.body.childNodes.length-1]);},addModule:function(s,f){this.M[s]=f;},
...
var opPre="/trial/639/p";
var opSfx="/content.js";

var opExperiments={}; //opExperiments Object that will contain all the Experiments/Trials running accross the website as properties.

//I have added one experiment example, calling it "test page 1"......  7 May 2010 - add code to make this pass in the match method, and you figure it out.
opExperiments["test page 1"]={"method":"var","match":false,"criteria":["test page 1"],unique":"/testpage.c04/2","throttle":1000,"gum":"op639testpagegum","domain":optimost.SLD(),"expire":2592000,"modules":["body"]};


var opModulesArray=new Array();
//Page Indentifier above Autonomy-Optimost include indicates what page we are on.  
//The code below will, if this is an experiment page, will add a script tag to the DOM that makes a call (Trial code call) to the Optimost servers.
if (typeof opPageId != "undefined"){
	var len=opPageId.length;
	for(var i=0;i<len;i++){
		if(opExperiments[opPageId[i]]!=null){
			(function(){var _o=optimost;_o.U=opPro+opPre+opExperiments[opPageId[i]].unique+opSfx;
			_o.ST="script";_o.SA={"type":"text/javascript"};_o.B();_o.R(opExperiments[opPageId[i]].throttle,opExperiments[opPageId[i]].gum,opExperiments[opPageId[i]].domain,opExperiments[opPageId[i]].expire);})();
			for(var j=0;j<opExperiments[opPageId[i]].modules.length;j++)opModulesArray.push(opExperiments[opPageId[i]].modules[j])
		}	
	}
}	
